...
if (Meteor.isServer) {
  describe('counters method', function() {
    ...
    it('can increase a counter', async function() {
      ...
      const stubbedUserId = Accounts.createUser({ email: 'test@user.com', password: 'test' });
      Roles.addUsersToRoles(stubbedUserId, 'user');
      await countersIncrease.run.call({ userId: stubbedUserId }, { _id: counterId });
    ...
    it('cannot increase a counter if not in "user" role', async function() {
      ...
      const stubbedUserId = Accounts.createUser({ email: 'not@in.com', password: 'user-role' });
      assert.throws(
        () => countersIncrease.run.call({ userId: stubbedUserId }, { _id: counterId }),
        Error,
